...
if _is_rocm:
    # -----------------------------------------------------------------------
    # AMD/ROCm backend: dispatch FP8 GEMM to AITER CK kernels (gfx942/gfx950)
    # -----------------------------------------------------------------------
    from ._amd_backend import (
        fp8_gemm_nt, fp8_gemm_nn, fp8_gemm_tn, fp8_gemm_tt,
        set_num_sms, get_num_sms,
        set_tc_util, get_tc_util,
        set_ignore_compile_dims, set_block_size_multiple_of,
        set_pdl, get_pdl,
    )
    # Aliases matching the NVIDIA API surface
    fp8_fp4_gemm_nt = fp8_gemm_nt
    fp8_fp4_gemm_nn = fp8_gemm_nn
    fp8_fp4_gemm_tn = fp8_gemm_tn
    fp8_fp4_gemm_tt = fp8_gemm_tt
else:
    # -----------------------------------------------------------------------
    # NVIDIA/CUDA backend: original compiled extension path
    # -----------------------------------------------------------------------
    # Configs
    from . import _C
    from ._C import (
    ...
    # Mega kernels (NVIDIA only — depends on _c and symmetric memory)
    from .mega import (
        SymmBuffer,
...
if not _is_rocm:
    # Initialize CPP modules (NVIDIA only)
