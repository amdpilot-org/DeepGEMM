...
try:
    ...
    import warnings
    warnings.warn(
        "DeepGEMM C++ extension (_C) is not available. "
        "Falling back to AITER baseline on AMD GPUs.",
        RuntimeWarning,
    )
...
# AITER baseline fallback for AMD GPUs
if _C is None:
    try:
        from .aiter_baseline import (
            fp8_gemm_nt, fp8_gemm_nn, fp8_gemm_tn, fp8_gemm_tt
        )
    except Exception as e:
        import warnings
        warnings.warn(f"Failed to load AITER baseline fallback: {e}", RuntimeWarning)
